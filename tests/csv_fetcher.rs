use goose::data::{CsvBarFetcher, CsvCalendarFetcher, Fetcher};
use goose::error::{Error, FetchError};
use std::io::Cursor;

const CSV: &str = "symbol,date,open,high,low,close,volume,amount
AAPL,2026-06-12,10,11,9.5,10.5,1000,10500
...
fn csv_fetcher_rejects_invalid_headers_and_batch_size() {
    let invalid_headers = "date,symbol,open,high,low,close,volume,amount\n";
